...
const addToCart = AsyncHandler(async(req, res) => {
     ...
     const populateUser = await User.findById(userId).populate('cart.product')

     return res.status(200).json(
        new ApiResponse(200, populateUser.cart,'items added to cart')
     )
})
...
const removeFromCart = AsyncHandler(async(req, res) => {
    ...
    return res.status(200).json(
        new ApiResponse(200, populatedUser.cart,'items removed from cart')
    )
})
